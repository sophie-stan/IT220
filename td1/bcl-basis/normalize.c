...
float normalize (const float max, const float min, const float vmax, const float vmin, const float value){
    return ((max-min)/(vmax-vmin))*value+(min*vmax-max*vmin)/(vmax-vmin);
}
    ...
    for(int p=0; p<rows; p++){
        for(int q=0; q<cols; q++){
            for (int k=0;k<3;k++){ 
                ...
                const float ip = (float) normalize(max, min, Max[k], Min[k], val);
